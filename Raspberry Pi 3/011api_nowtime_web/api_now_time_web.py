...
def add_data():
	...
	if request.method == 'GET':
		...
		nowdate = ("%s/%s/%s" % (i.year, i.month, i.day))	#GET模式傳資料指令
		nowtime = ("%s:%s:%s" % (i.hour,i.minute,i.second))	#GET模式傳資料指令
	else:
		name = str(request.form["name"])				#POST模式傳資料指令
		nowdate = str(request.form["nowdate"])		#POST模式傳資料指令
		nowtime = str(request.form["nowtime"])		#POST模式傳資料指令
	...
	try:
		...
		cursor.execute(sql_query, (name,nowdate,nowtime))
		conn.commit()
